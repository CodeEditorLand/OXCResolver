...
pub struct ResolveOptions {
	...
	///
	/// If `enforce_extension` is set to [EnforceExtension::Enabled], resolution
	/// will not allow extension-less files. This means `require('./foo.js')`
	/// will resolve, while `require('./foo')` will not.
	///
	...
	///
	/// Explicitly set the value to [EnforceExtension::Disabled] to disable this
	/// automatic behavior.
	///
	...
	/// main files are not resolved for it (they are still resolved for internal
	/// requests).
	///
	...
	/// their symlinked location. Note that this may cause module resolution to
	/// fail when using tools that symlink packages (like npm link).
	///
	/// Default `true`
	pub symlinks: bool,
...
impl ResolveOptions {
	...
	/// Adds a single extension to the list of extensions. Extension must start
	/// with a `.`
	///
	/// ## Examples
	///
	...
	pub(crate) fn sanitize(mut self) -> Self {
		...
		// Set `enforceExtension` to `true` when [ResolveOptions::extensions] contains
		// an empty string. See <https://github.com/webpack/enhanced-resolve/pull/285>
...
mod test {
	...
	use super::{AliasValue, EnforceExtension, ResolveOptions, Restriction, TsconfigOptions, TsconfigReferences};
