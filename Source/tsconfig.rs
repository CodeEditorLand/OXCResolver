...
pub trait TsConfig: Sized {
	...
	fn extend_tsconfig(&mut self, tsconfig: &Self) {
		...
		if compiler_options.paths().is_none() {
			compiler_options.set_paths_base(
				compiler_options
					.base_url()
					.map_or_else(|| tsconfig.compiler_options().paths_base().to_path_buf(), Path::to_path_buf),
			);
		...
		if compiler_options.experimental_decorators().is_none() {
			if let Some(experimental_decorators) = tsconfig.compiler_options().experimental_decorators() {
