...
use crate::{
	FileMetadata, FileSystem, PackageJsonSerde, ResolveError, ResolveOptions, TsConfig, TsConfigSerde,
...
impl<Fs: FileSystem> Cache for FsCache<Fs> {
	...
	fn get_tsconfig<F: FnOnce(&mut TsConfigSerde) -> Result<(), ResolveError>>(
		...
		let mut tsconfig = TsConfigSerde::parse(root, &tsconfig_path, &mut tsconfig_string)
			.map_err(|error| ResolveError::from_serde_json_error(tsconfig_path.to_path_buf(), &error))?;
				...
				let res = path.parent().map_or_else(
					...
					|parent| {
						self.canonicalize_impl(parent).and_then(|parent_canonical| {
							let normalized =
								parent_canonical.normalize_with(path.path().strip_prefix(parent.path()).unwrap(), self);
							...
							if self.fs.symlink_metadata(path.path()).is_ok_and(|m| m.is_symlink) {
								...
								debug_assert!(false, "Failed to get path parent for {:?}.", normalized.path());
							}
