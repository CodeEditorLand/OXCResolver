...
pub use crate::{
	...
	options::{Alias, AliasValue, EnforceExtension, ResolveOptions, Restriction, TsconfigOptions, TsconfigReferences},
	package_json::{
		ImportsExportsArray, ImportsExportsEntry, ImportsExportsKind, ImportsExportsMap, PackageJson, PackageType,
	},
	path::PathUtil,
...
impl<C: Cache> ResolverGeneric<C> {
	...
	///
	/// * See [ResolveError]
	pub fn resolve<P: AsRef<Path>>(&self, directory: P, specifier: &str) -> Result<Resolution<C>, ResolveError> {
	...
	fn resolve_tracing(&self, directory: &Path, specifier: &str, ctx: &mut Ctx) -> Result<Resolution<C>, ResolveError> {
	...
	fn resolve_impl(&self, path: &Path, specifier: &str, ctx: &mut Ctx) -> Result<Resolution<C>, ResolveError> {
		...
		let package_json = cached_path.find_package_json(&self.options, self.cache.as_ref(), ctx)?;
	...
	fn require(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<C::Cp, ResolveError> {
	...
	fn require_without_parse(
		...
		let result = match Path::new(specifier).components().next() {
			// 2. If X begins with '/'
			Some(Component::RootDir | Component::Prefix(_)) => self.require_absolute(cached_path, specifier, ctx),
			// 3. If X begins with './' or '/' or '../'
			Some(Component::CurDir | Component::ParentDir) => self.require_relative(cached_path, specifier, ctx),
			// 4. If X begins with '#'
	...
	fn require_absolute(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<C::Cp, ResolveError> {
	...
	// 3. If X begins with './' or '/' or '../'
	fn require_relative(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<C::Cp, ResolveError> {
		// Make sure only relative or normal paths gets called
		debug_assert!(
			Path::new(specifier)
				.components()
				.next()
				.is_some_and(|c| matches!(c, Component::CurDir | Component::ParentDir | Component::Normal(_)))
		);
	...
	fn require_hash(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<C::Cp, ResolveError> {
	...
	fn require_bare(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<C::Cp, ResolveError> {
	...
	fn load_package_imports(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		// 1. Find the closest package scope SCOPE to DIR.
		// 2. If no scope was found, return.
		let Some((_, package_json)) = cached_path.find_package_json(&self.options, self.cache.as_ref(), ctx)? else {
			return Ok(None);
		};
	...
	fn load_as_directory(&self, cached_path: &C::Cp, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		...
		if !self.options.description_files.is_empty() {
			...
			if let Some((_, package_json)) = self.cache.get_package_json(cached_path, &self.options, ctx)? {
	...
	fn load_as_file_or_directory(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
	...
	fn load_extensions(&self, path: &C::Cp, extensions: &[String], ctx: &mut Ctx) -> ResolveResult<C::Cp> {
			...
			match restriction {
				Restriction::Path(restricted_path) => {
					if !is_inside(path, restricted_path) {
						return Err(ResolveError::Restriction(path.to_path_buf(), restricted_path.clone()));
					}
				},
	...
	fn load_browser_field_or_alias(&self, cached_path: &C::Cp, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		if !self.options.alias_fields.is_empty() {
			...
			{
				if let Some(path) = self.load_browser_field(cached_path, None, &package_url, &package_json, ctx)? {
					return Ok(Some(path));
				}
			}
		}
		...
		if !self.options.alias.is_empty() {
			...
			if let Some(path) = self.load_alias(cached_path, &alias_specifier, &self.options.alias, ctx)? {
				return Ok(Some(path));
			}
		}
		Ok(None)
	...
	fn load_node_modules(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
			...
			for cached_path in std::iter::successors(Some(cached_path), |p| p.parent()) {
				...
				if !package_name.is_empty() {
					...
					if self.cache.is_dir(&cached_path, ctx) {
						...
						if let Some(path) = self.load_package_exports(specifier, subpath, &cached_path, ctx)? {
							return Ok(Some(path));
						}
					} else {
	...
	#[cfg(feature = "yarn_pnp")]
	fn load_pnp(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> Result<Option<C::Cp>, ResolveError> {
		let Some(pnp_manifest) = &self.options.pnp_manifest else {
			return Ok(None);
		};
		let resolution = pnp::resolve_to_unqualified_via_manifest(pnp_manifest, specifier, cached_path.path());
		match resolution {
			Ok(pnp::Resolution::Resolved(path, subpath)) => {
				let cached_path = self.cache.value(&path);
				let export_resolution =
					self.load_package_exports(specifier, &subpath.unwrap_or_default(), &cached_path, ctx)?;
	...
	fn get_module_directory(&self, cached_path: &C::Cp, module_name: &str, ctx: &mut Ctx) -> Option<C::Cp> {
		if module_name == "node_modules" {
			cached_path.cached_node_modules(self.cache.as_ref(), ctx)
		} else if cached_path.path().components().next_back() == Some(Component::Normal(OsStr::new(module_name))) {
	...
	fn load_package_exports(
		...
		//    return.
		let Some((_, package_json)) = self.cache.get_package_json(cached_path, &self.options, ctx)? else {
			return Ok(None);
		};
		...
		for exports in package_json.exports_fields(&self.options.exports_fields) {
			if let Some(path) = self.package_exports_resolve(cached_path, &format!(".{subpath}"), &exports, ctx)? {
	...
	fn load_package_self(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		// 1. Find the closest package scope SCOPE to DIR.
		// 2. If no scope was found, return.
	...
	fn resolve_esm_match(&self, specifier: &str, cached_path: &C::Cp, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
	...
	fn load_alias(&self, cached_path: &C::Cp, specifier: &str, aliases: &Alias, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		for (alias_key_raw, specifiers) in aliases {
			...
			for r in specifiers {
				match r {
					...
					AliasValue::Ignore => {
						let cached_path = cached_path.normalize_with(alias_key, self.cache.as_ref());
						return Err(ResolveError::Ignored(cached_path.to_path_buf()));
					},
				}
			}
			if should_stop {
				return Err(ResolveError::MatchedAliasNotFound(specifier.to_string(), alias_key.to_string()));
			}
		}
		Ok(None)
	}
	...
	fn load_alias_value(
		...
		if request != alias_value && !request.strip_prefix(alias_value).is_some_and(|prefix| prefix.starts_with('/')) {
		...
		self.cache.get_tsconfig(root, path, |tsconfig| {
			...
			for extended_tsconfig_path in extended_tsconfig_paths {
				let extended_tsconfig =
					self.load_tsconfig(/* root */ false, &extended_tsconfig_path, &TsconfigReferences::Disabled)?;
	...
	fn load_tsconfig_paths(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
	...
	/// PACKAGE_RESOLVE(packageSpecifier, parentURL)
	fn package_resolve(&self, cached_path: &C::Cp, specifier: &str, ctx: &mut Ctx) -> ResolveResult<C::Cp> {
		let (package_name, subpath) = Self::parse_package_specifier(specifier);
		...
		if let Some(map) = exports.as_map() {
			...
			for key in map.keys() {
				...
				if has_dot && without_dot {
					return Err(ResolveError::InvalidPackageConfig(package_url.path().join("package.json")));
				}
			}
		}
	...
	fn package_imports_exports_resolve<'a, Io: ImportsExportsMap<'a>>(
		...
		if !match_key.contains('*') {
			...
			if let Some(target) = match_obj.get(match_key) {
				...
				return self.package_target_resolve(package_url, match_key, &target, None, is_imports, conditions, ctx);
			}
		}
	...
	fn parse_package_specifier(specifier: &str) -> (&str, &str) {
		...
		let package_name = separator_index.map_or(specifier, |separator_index| &specifier[..separator_index]);
		...
		// }
		let package_subpath = separator_index.map_or("", |separator_index| &specifier[separator_index..]);
