...
impl PackageJsonSerde {
	/// # Panics
	/// # Errors
	pub(crate) fn parse(path: PathBuf, realpath: PathBuf, json: &str) -> Result<Self, serde_json::Error> {
		...
		if let Some(json_object) = raw_json.as_object_mut() {
			...
			package_json.r#type = json_object.get("type").and_then(|ty| serde_json::from_value(ty.clone()).ok());
	...
	fn get_value_by_path<'a>(fields: &'a serde_json::Map<String, JSONValue>, path: &[String]) -> Option<&'a JSONValue> {
		if path.is_empty() {
			return None;
		}
	...
	fn browser_fields<'a>(&'a self, alias_fields: &'a [Vec<String>]) -> impl Iterator<Item = &'a JSONMap> + 'a {
...
impl<'a> Iterator for ImportsExportsSerdeMapIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		self.inner
			.next()
			.map(|(key, value)| (key.as_str(), ImportsExportsSerdeEntry(value)))
	}
}
